...
def run(cfg: Config, posts: list[dict] | None = None) -> dict:
    if posts is None:
        # 1. Fetch PTT posts
        logger.info("Step 1/5: Fetching PTT/%s posts (threshold=%d)", cfg.ptt_board, cfg.ptt_push_threshold)
        posts = ptt.fetch_popular_posts(cfg.ptt_board, cfg.ptt_push_threshold, cfg.max_posts_per_run)
    else:
        logger.info("Step 1/5: Using %d pre-fetched posts (local scrape)", len(posts))
    ...
    # 2. Deduplicate
    logger.info("Step 2/5: Deduplicating against processed posts")
    ...
    logger.info("Step 3/5: Summarizing and analyzing sentiment via Gemini")
    ...
    logger.info("Step 4/5: Generating community post text and image prompt")
    time.sleep(4)
    ...
    # 5. Imagen 3: generate 9:16 image and upload to GCS
    logger.info("Step 5/5: Generating 9:16 image via Imagen 3")
    image_bytes = image.generate(image_prompt, cfg.gcp_project)
    blob_path = gcs.upload_image(cfg.gcs_bucket, image_bytes)

    logger.info("Pipeline complete — content ready for local posting")
    return {
        "status": "ready",
        "post_text": post_text,
        "image_blob_path": blob_path,
        "processed_urls": [p["url"] for p in new_posts],
    }
